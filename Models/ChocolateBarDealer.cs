...
public class ChocolateBarDealer(DatabaseCredentials credentials) : GenericDealer(credentials) {
    ...
    public async Task<IEnumerable<ChocolateBar>> GetAll() {
        NpgsqlDataReader reader = await RetrieveData("SELECT * FROM chocolate_bars;");
    ...
    private ChocolateBar GetBarFromFullRow(NpgsqlDataReader reader) {
        ChocolateBar bar = new() {
            ...
            CompanyLocation = (string)reader.GetValue(6),
            Rating = (float)reader.GetValue(7),
            BeanType = (string)reader.GetValue(8),
            BroadOrigin = (string)reader.GetValue(9)
        };
        if (bar == null) {
